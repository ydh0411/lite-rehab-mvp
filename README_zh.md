...
最终多模态 benchmark 模型确定为 **Lite-ActionMAE**。它是面向传感器输入的轻量
ActionMAE 改写版，与五个紧凑 baseline 在正常、Pose 缺失和 IMU 缺失条件下进行
三随机种子比较；模型选择同时报告鲁棒性、稳定性、延迟、参数量和正常条件性能。
完整方法与重画命令见
[`experiments/lite_benchmark/README.md`](experiments/lite_benchmark/README.md)。

该实验选择不会直接覆盖现场 Demo checkpoint：mRI benchmark 输入并不等同于当前
单腕 MPU6050 六轴数据。在获得设备对齐的同步训练数据并重新训练 Lite-ActionMAE
之前，硬件 Demo 继续使用已经真机验证的 IMU CNN-BiGRU/规则回退。
...
| 多模态 benchmark | Lite-ActionMAE 最终实验模型及五个紧凑 baseline | 离线实验已实现；待训练设备对齐的在线 checkpoint |
...
flowchart LR
    ...
    DASH --> MODEL["IMU CNN-BiGRU 回退<br/>Lite-ActionMAE 最终 benchmark"]
